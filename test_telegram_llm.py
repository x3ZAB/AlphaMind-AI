...
class FakeStockService(StockAnalysisService):
    async def analyze_query(self, query: str) -> dict:
        return {
            "company": {"name": "NVIDIA", "ticker": query},
            "quote": {"c": 100},
        }


class FakeLLMService(ConfiguredLLMService):
    def __init__(self, response: str = "AI analysis response") -> None:
...
def test_existing_user_resolved_by_telegram_id() -> None:
    ...
    try:
        existing = User(telegram_id="123", username="existing")
        db.add(existing)
        db.commit()

        resolved = UserRepository(db).get_or_create_by_telegram_id(
            "123",
            username="new-name",
        )
...
def test_repeated_user_resolution_does_not_duplicate() -> None:
    ...
    try:
        ...
        first = repository.get_or_create_by_telegram_id("789")
        second = repository.get_or_create_by_telegram_id("789")
...
async def test_english_analysis_reaches_llm() -> None:
    ...
    assert response == "AI analysis response"
    assert llm.received_configuration is user.llm_configuration
    assert "NVDA" in llm.received_messages[1]["content"]
...
async def test_analysis_handler_creates_user_and_preserves_missing_config_message() -> None:
    ...
    class FakeUpdate:
        def __init__(self) -> None:
            self.message = FakeReplyMessage("analyze NVDA")
    ...
    update = FakeUpdate()
    try:
        await telegram.analysis_message(update, "analyze NVDA")
        user = db.query(User).filter_by(telegram_id="321").one()
    finally:
...
async def test_missing_configuration_is_user_friendly() -> None:
    ...
    try:
        await service.analyze(user, "حلل NVDA")
    except MissingLLMConfigurationError as error:
        message = user_facing_analysis_error(error)
    else:
        raise AssertionError("Missing configuration did not raise")
...
async def test_provider_failures_are_user_friendly() -> None:
    assert "supported" in user_facing_analysis_error(
        UnknownLLMProviderError("internal provider")
    )
    assert "unavailable" in user_facing_analysis_error(
        LLMProviderError("internal provider failure")
    )
...
async def test_successful_response_does_not_expose_api_key() -> None:
    ...
    llm = FakeLLMService("Balanced analysis without credentials")
    service = TelegramAnalysisService(
        stock_service=FakeStockService(),
        llm_service=llm,
        analysis_service=LLMAnalysisService(),
    ...
    response = await service.analyze(user, "حلل NVDA")

    assert api_key not in response
    assert response == "Balanced analysis without credentials"
...
async def test_start_help_and_plain_ticker_lookup_remain_available() -> None:
    ...
    await telegram.start(start_update, None)
    assert "Welcome" in start_update.message.replies[0]

    help_update = FakeUpdate("/help")
    await telegram.help_command(help_update, None)
    ...
    try:
        ...
        await telegram.stock_message(ticker_update, None)
    finally:
